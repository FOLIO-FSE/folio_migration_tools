...
from marc_to_folio import mapping_file_transformation
...
class BibsRulesMapper(RulesMapperBase):
    ...
    def report_marc_stats(
        ...
        if marc_field.tag in ignored_subsequent_fields:
            mapped = False
        self.report_legacy_mapping(marc_field.tag, True, mapped, True)
    ...
    def report_bad_tags(self, marc_field, bad_tags, index_or_legacy_id):
        if (
            ...
            self.add_to_migration_report(message, marc_field.tag)
    ...
    def filter_langs(
        ...
        for language_value in language_values:
            if (
                ...
                self.add_to_migration_report(
                    "Unrecognized language codes in records", language_value
                )
