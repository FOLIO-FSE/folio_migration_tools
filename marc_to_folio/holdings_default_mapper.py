...
class HoldingsDefaultMapper:
    ...
    def __init__(self, folio, instance_id_map, location_map=None):
        ...
        self.locations = self.folio.folio_get_all("/locations", "locations")
        print(f"LOCATIONS: {len(self.locations)}")
        self.stats = {
            "number of bib id not in map": 0,
            "multiple 852s": 0,
            "tailing_zeroes_match": 0,
        }
        self.locations_map = {}
        if any(location_map):
            ...
            print(
                (
                    f"{len(location_map)} locations rows and "
    ...
    def remove_from_id_map(self, marc_record):
        ...
        if id_key in self.instance_id_map:
            print(f"removing Holdings id {id_key} from map")
            del self.instance_id_map[id_key]

    def get_loc_id(self, loc_code):
        loc = next((l["id"] for l in self.locations if loc_code == l["code"]), "")
        if not loc:
            self.unmapped_locations[loc_code] = (
                self.unmapped_locations.get(loc_code, 0) + 1
            )
    ...
    def wrap_up(self):
        ...
        sorted_tags = {k: self.tags[k] for k in sorted(self.tags)}
    ...
    def get_instance_id(self, marc_record):
        try:
            ...
            elif old_instance_id.lstrip("0") in self.instance_id_map:
                self.stats["tailing_zeroes_match"] += 1
                return self.instance_id_map[old_instance_id.lstrip("0")]["id"]
            else:
                logging.warn(f"Old instance id not in map: {old_instance_id}")
                raise Exception(
                    f"Old instance id not in mapp: {old_instance_id} {old_instance_id.lstrip('0')}"
                )
        except Exception as ee:
            self.stats["number of bib id not in map"] += 1
            raise ValueError(
                f'Error getting new FOLIO Instance {marc_record["001"]} - {ee}'
            )
    ...
    def get_location(self, marc_record):
        ...
        if "852" in marc_record and "c" in marc_record["852"]:
            ...
            self.legacy_locations[loc_code] = self.legacy_locations.get(loc_code, 0) + 1
            ...
            self.folio_locations[folio_id] = self.folio_locations.get(folio_id, 0) + 1
            return folio_id
        else:
            raise ValueError(
                "no 852 $c in {}. Unable to parse location".format(
