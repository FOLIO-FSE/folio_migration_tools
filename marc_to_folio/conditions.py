...
class Conditions():
    ...
    def condition_set_identifier_type_id_by_value(self, value, parameter, marc_field):
        ...
        if 'oclc_regex' in parameter:
            if re.match(parameter['oclc_regex'], value):
                return next((f['id'] for f in self.folio.identifier_types
    ...
    def condition_set_contributor_type_id(self, value, parameter, marc_field):
        ...
        if not self.default_contributor_type:
            self.default_contributor_type = next(ct for ct in self.folio.contributor_types
                                                 if ct['code'] == 'ctb')
    ...
    def condition_set_instance_type_id(self, value, parameter, marc_field):
        ...
        if not v:
            raise ValueError(
                f'no matching instance_types {value} {marc_field}')
        return v
    ...
    def condition_set_instance_format_id(self, value, parameter, marc_field):
        ...
        if not v:
            raise ValueError(
                f'no matching instance_formats {value} {marc_field}')
        return v
    ...
    def condition_set_electronic_access_relations_id(self, value, parameter, marc_field):
        ...
        if not self.electronic_access_relationships:
            raise ValueError(
                'No electronic_access_relationships setup in tenant')
        v = next((f['id'] for f
