...
class DefaultMapper:
    ...
    def parse_bib(self, marc_record, record_source):
        ...
        rec = {
            ...
            "publicationFrequency": list(
                set(self.get_publication_frequency(marc_record))
            ),
    ...
    def get_instance_type_id(self, marc_record):
        ...
        return next(i["id"] for i in self.folio.instance_types if code == i["code"])
    ...
    def get_contributors(self, marc_record):
        ...
        Ids"""
        fields = {
            "100": {"subfields": "abcdq", "nameTypeId": "Personal name"},
            "110": {"subfields": "abcdn", "nameTypeId": "Corporate name"},
            "111": {"subfields": "abcd", "nameTypeId": "Meeting name"},
            "700": {"subfields": "abcdq", "nameTypeId": "Personal name"},
            "710": {"subfields": "abcdn", "nameTypeId": "Corporate name"},
            "711": {"subfields": "abcd", "nameTypeId": "Meeting name"},
        }
        first = 0
        for field_tag in fields:
            for field in marc_record.get_fields(field_tag):
                ...
                yield {
                    "name": re.sub(self.filter_last_chars, "", " ".join(subs)),
    ...
    def get_alt_titles(self, marc_record):
        ...
        for field_tag in self.alt_title_map:
            for field in marc_record.get_fields(field_tag):
                yield {
                    "alternativeTitleTypeId": self.alt_title_map[field_tag][0],
                    "alternativeTitle": " - ".join(
                        field.get_subfields(*self.alt_title_map[field_tag][1])
                    ),
                }
    ...
    def get_publication(self, marc_record):
        ...
        for field in marc_record.get_fields("260"):
            ...
            yield {
                "publisher": self.get_filtered_subfield(field, "b"),
                "place": self.get_filtered_subfield(field, "a"),
                "dateOfPublication": re.sub(self.filter_chars_dop, str(""), dop).strip()
                or "",
            }
    ...
    def get_classifications(self, marc_record):
        ...
        def get_class_type_id(x):
            return next(
                (f["id"] for f in self.folio.class_types if f["name"] == x), None
            )
        ...
        for field_tag in fields:
            for field in marc_record.get_fields(field_tag):
                ...
                if class_type:
                    yield {
                        "classificationTypeId": get_class_type_id(fields[field_tag][0]),
                        "classificationNumber": " ".join(
                            field.get_subfields(*fields[field_tag][1])
                        ),
                    }
                else:
                    print(
                        "No classification type for {} ({}) for {}.".format(
                            field.tag,
                            field.format_field(),
                            marc_record["001"].format_field(),
                        )
            ...
            for field in marc_record.get_fields(tag):
                for subfield in field.get_subfields(*list(subfields)):
                    if identifier_type_id:
                        yield {
                            "identifierTypeId": identifier_type_id,
                            "value": subfield,
                        }
