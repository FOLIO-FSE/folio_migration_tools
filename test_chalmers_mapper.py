...
class TestChalmersMapper(unittest.TestCase):
    ...
    def do_map(self, file_name, xpath, message):
        ...
        file_path = f"./tests/test_data/chalmers/{file_name}"
    ...
    def test_simple_title(self):
        ...
        self.assertEqual(
            "6312d172-f0cf-40f6-b27d-9fa8feaf332f", record[0]["instanceTypeId"]
        )
        self.assertEqual(
            "9d18a02f-5897-4c31-9106-c9abb5c7ae8b", record[0]["modeOfIssuanceId"]
        )
    ...
    def test_permanent_location_two_holdings(self):
        ...
        permanenent_loc_ids = [
            h["permanentLocationId"] for h in self.mapper.holdings_map.values()
        ]
        callNumbers = [h["callNumber"] for h in self.mapper.holdings_map.values()]
        holdingsStatements = [
            h["holdingsStatements"][0] for h in self.mapper.holdings_map.values()
        ]
        ...
        self.assertIn(
            {"statement": "Årg. 24-40 (1986-2002)", "note": ""}, holdingsStatements
        )
    ...
    def test_identifiers(self):
        message = (
            "Should add identifiers: 010, 019, 020, 022, 024, 028, 035 and local IDs"
        )
        ...
        identifier_values = (i["value"] for i in record[0]["identifiers"])
