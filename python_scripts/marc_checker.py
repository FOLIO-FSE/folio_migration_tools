...
def main():
    start = time.time()
    failed_files = []
    files = [join(sys.argv[1], f) for f in listdir(sys.argv[1])
            ...
            with open(file_name, 'rb') as marc_file:
                ...
                for idx, marc_record in enumerate(reader):
                    ...
                    if i % 1000 == 0:
                        ...
                        print(
                            f"{elapsed_formatted} Number of records: {i}, Failed: {failed}", flush=True)
        except UnicodeDecodeError as decode_error:
            failed += 1
            print(
                f"UnicodeDecodeError in {file_name} for index {idx} (after record id {marc_record['001'].data}) {decode_error}", flush=True)
