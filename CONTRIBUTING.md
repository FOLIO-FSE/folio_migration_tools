...
So, if you are to test a method named _condition_trim_period_ in the _conditions.py_ file, your test file should be named _test_conditions.py_ and the test method should be named _test_condition_trim_period_
...
The exception to this is the test suite in _test_rules_mapper_bibs.py_ that needs to be rewritten long-term, but that will remain in the current form as is. So if you want to test the tools agains real-world data and a tenant, then this is the place to do it.
...
The documentation should now have been built in the docs/\_build folder. Open up the _index.html_ file in a browser to see how it looks. Note that the formatting on Read the docs will make it look different, but that the resulting HTML should be the same
