...
def main():
    ...
    parser.add_argument("-items_mapper_file", "-it",
                        help=(""))
    ...
    files = [f for f in listdir(args.records_file)
    ...
    try:
        mapper_name = next((m for m in mappers
                            if args.mapper and args.mapper in m), "ItemsDefaultMapper")
        print(mapper_name)
    ...
    with open(os.path.join(args.result_path, 'folio_items.json'), 'w+') as results_file:
        ...
        for file_name in files:
            ...
            try:
                ...
                print(
                    f"UnicodeDecodeError in {file_name} for index")
